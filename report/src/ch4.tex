\chapter{Authenticated response and containment}
\label{ch:ch4}
...
the Cilium isolation policy before enabling automatic response. The historical kmod
installation in Chapter 2 is not the current supported quickstart.
...
Unit tests and Docker checks are recorded separately from live-network checks in
the README validation section. No end-to-end Cilium result is asserted for the current OrbStack
environment. Sample logs elsewhere in this guide are illustrative historical examples.
...
Deployment alone is not a complete cleanup. Remove only lab-owned resources and
